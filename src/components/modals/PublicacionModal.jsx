...
export const PublicacionModal = ({
  ...
  setPublicaciones,
  setCantPublicaciones,
  actualizarCantidadLikes,
  ...
  const [mostrarOpcionesComentarioPropio, setMostrarOpcionesComentarioPropio] =
    useState(false);
  ...
  const obtenerComentarios = async () => {
    try {
      const comentariosRef = collection(
        db,
        `Publicaciones/${publicacion.id}/Comentarios`
      );
    ...
    try {
      ...
      if (!esPropia) {
        ...
        const notificacionRef = doc(
          collection(
            db,
            `Usuarios/${publicacion.idUsuario}/NotificacionesComentarios`
          )
        );
      ...
      console.error(
        "Error al comentar la publicación (comentarPublicacion):",
        error
      );
    }
  };
  ...
  const validarLikePrevio = async () => {
    ...
    const q = query(
      likesCollectionRef,
      where("idUsuario", "==", idUsuarioAutenticado)
    );
    const querySnapshot = await getDocs(q);
  ...
  const nextImage = () => {
    setCurrentImageIndex(
      (prevIndex) => (prevIndex + 1) % publicacion.fileUrls.length
    );
  };
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center bg-white bg-opacity-50 z-50">
      <div
        className="bg-white text-black rounded-lg shadow-lg w-full max-w-6xl mx-4 flex relative"
        style={{ height: "800px" }}
      >
        {/* Sección de imagen */}
        <div className="w-2/3 relative flex flex-col">
          <div
            className="img-co"
            style={{ width: "100%", height: "700px", background: "white" }}
          >
            <img
        ...
        <div className="w-1/3 p-4 flex flex-col">
          ...
          <div className="mb-4">
            ...
            <p className="text-sm text-gray-400">
              Ubicación: {publicacion.location}
            </p>
                    ...
                    <div className="max-w-full">
                      <div className="text-sm flex flex-col">
                        <span
                          ...
                          onClick={() =>
                            handlePerfilClick(comentario.usuario.id)
                          }
                        >
          ...
          <div className="mb-2" style={{ color: "#a4a4a4", fontSize: "15px" }}>
            <span onClick={() => mostrarLikes(publicacion.id)}>
