...
export const PerfilCardGeneral = ({ idUsuarioE }) => {
  ...
  const [
    ...
    setMostrarModalSolicitudSeguimiento,
  ] = useState(false);

  //Mostrar el modal para interactuar con botón "Pendiente" (¿eliiminar solicitud de amistad enviada?).
  const [mostrarModalPendienteSolicitud, setMostrarModalPendienteSolicitud] =
    useState(false);
  ...
  const resetPassword = async () => {
    await sendPasswordResetEmail(auth, usuarioDetalles.email).then(() => {
      Swal.fire({
        title: "Email enviado",
    ...
    if (file) {
      ...
      if (!extensiones.includes(file.type)) {
        ...
        reader.onload = (e) => {
          ...
          Swal.fire({
            title:
              "Por favor, confirme si desea cambiar la imagen de perfil por la mostrada arriba.",
  ...
  const eliminarFotoPerfil = async () => {
    usuarioDetalles.foto =
      "https://firebasestorage.googleapis.com/v0/b/mochimap-proyecto.appspot.com/o/profile-circle-icon-256x256-cm91gqm2.png?alt=media&token=5b2a71ae-e78d-40c4-b2c7-0e07511ad2a3";
    await onUpdate("Usuarios", idUsuario, usuarioDetalles);
    refrescar();
    setMostrarModalFoto(false);
  };
  ...
  const dejarDeSeguir = async () => {
    try {
      const refSeguidor = doc(
        db,
        `Usuarios/${idUsuarioE}/Seguidores/${idUsuario}`
      );
      await deleteDoc(refSeguidor);

      const refSeguido = doc(
        db,
        `Usuarios/${idUsuario}/Siguiendo/${idUsuarioE}`
      );
    ...
    try {
      ...
      if (docUsuarioSnap.exists()) {
        ...
        if (usuarioData.privacidad === "publica") {
          ...
          const refSolicitud = doc(
            db,
            `Usuarios/${idUsuarioE}/Solicitudes/${idUsuario}`
          );
          await setDoc(refSolicitud, { id: idUsuario });
          const estado = await verificarSiHasEnviadoSolicitud(
            idUsuario,
            idUsuarioE
          );
  ...
  const aceptarSolicitud = async () => {
    try {
      ...
      const refSolicitud = doc(
        db,
        `Usuarios/${idUsuario}/Solicitudes/${idUsuarioE}`
      );
      const refSeguidor = doc(
        db,
        `Usuarios/${idUsuario}/Seguidores/${idUsuarioE}`
      );
      const refSeguido = doc(
        db,
        `Usuarios/${idUsuarioE}/Siguiendo/${idUsuario}`
      );
      ...
      const estado = await verificarSiTeHaEnviadoSolicitud(
        idUsuario,
        idUsuarioE
      );
      ...
      if (user) {
        try {
          ...
          if (userDetails) {
            ...
            */
            const [loSigo, esPublico, sigue, haEnviado, teEnviado] =
              await Promise.all([
  ...
  const verificarSiTeSigue = async (idUsuarioAutenticado, idUsuarioPerfil) => {
    const ref = doc(
      db,
      `Usuarios/${idUsuarioPerfil}/Siguiendo/${idUsuarioAutenticado}`
    );
    const docSnap = await getDoc(ref);
    return docSnap.exists();
  };

  const verificarSiHasEnviadoSolicitud = async (
    idUsuarioAutenticado,
    idUsuarioPerfil
  ) => {
    const ref = doc(
      db,
      `Usuarios/${idUsuarioPerfil}/Solicitudes/${idUsuarioAutenticado}`
    );
    const docSnap = await getDoc(ref);
    return docSnap.exists();
  };

  const verificarSiTeHaEnviadoSolicitud = async (
    idUsuarioAutenticado,
    idUsuarioPerfil
  ) => {
    const ref = doc(
      db,
      `Usuarios/${idUsuarioAutenticado}/Solicitudes/${idUsuarioPerfil}`
    );
    const docSnap = await getDoc(ref);
    return docSnap.exists();
  };
  ...
  const obtenerDetallesUsuario = async (id) => {
    try {
      ...
      console.error(
        "Error al obtener detalles del usuario (obtenerDetallesUsuario): ",
        error
      );
    }
  };
  ...
  const obtenerPublicacionesUsuario = async (id) => {
    try {
      ...
      console.error(
        "Error al obtener publicaciones (obtenerPublicacionesUsuario): ",
        error
      );
      return [];
    }
  ...
  const obtenerCantSeguidores = async (id) => {
    try {
      const queryCantSeguidores = await getDocs(
        collection(db, `Usuarios/${id}/Seguidores`)
      );
      return queryCantSeguidores.size;
    } catch (error) {
      console.error(
        "Error al obtener cantidad de seguidores (obtenerCantSeguidores): ",
        error
      );
      return 0;
    }
  ...
  const obtenerCantSeguidos = async (id) => {
    try {
      const queryCantSeguidos = await getDocs(
        collection(db, `Usuarios/${id}/Siguiendo`)
      );
      return queryCantSeguidos.size;
    } catch (error) {
      console.error(
        "Error al obtener cantidad de seguidos (obtenerCantSeguidos): ",
        error
      );
      return 0;
    }
  ...
  const verificarSiLoSigo = async (idUsuarioAutenticado, idUsuarioPerfil) => {
    try {
      ...
      console.error(
        "Error al verificar si es un usuario seguido (verificarSiLoSigo): ",
        error
      );
      return false;
    }
  ...
  const verificarPrivacidad = async (id) => {
    try {
      ...
      console.error(
        "Error al verificar privacidad (verificarPrivacidad): ",
        error
      );
      return false;
    }
                        ...
                        <div className="p-4 text-black">
                          <div className="mb-5">
                            ...
                            <div className="p-4 bg-white">
                              <p className="mb-1">
                                {usuarioDetalles.biografia}
                              </p>
                            </div>
                          </div>
                          <div className="flex justify-between items-center mb-4">
                            <p
                              className={`lead fw-normal mb-0 cursor-pointer ${
                                activo === 1 ? "underline" : "hover:underline"
                              }`}
                              onClick={() => setActivo(1)}
                            >
                            ...
                            <p
                              className={`lead fw-normal mb-0 cursor-pointer ${
                                activo === 2 ? "underline" : "hover:underline"
                              }`}
                              onClick={
                                () => setActivo(2)
                          ...
                          {activo === 1 && (
                            <div className="flex flex-wrap">
                              {publicacionesOrdenadas.map((publicacion) => (
                                ...
                                >
                                  {publicacion.fileUrls &&
                                    publicacion.fileUrls.length > 0 && (
                                      <img
                          ...
                          {activo === 2 && (
                            <div className="flex flex-wrap">
                              <p>
                                Acá va la lógica para mapear las publicaciones
                                compartidas basándose en el idUsuario principal
                                que recibe este componente funcional
                              </p>
                            </div>
                          )}
                        </div>
        ...
        <div>
          {loSigo === true ? (
            ...
            privacidad === false ? (
              ...
              <p>
                El usuario no es tu amigo y el estado de privacidad es
                desconocido.
              </p>
            )
          ) : (
            <p>
              No se puede determinar el estado de amistad del usuario. /
              Determinando estado de amistad
            </p>
          )}
        </div>
      )}
